...
function stripThinkTags(text: string): string {
	...
	return idx >= 0 ? text.slice(idx + 8) : text;
}
...
export function createNoteSearcher(
	...
	return async function noteSearcher(
		...
		const keywordResponse = await llm.invoke([
			new SystemMessage(EXTRACT_KEYWORDS_PROMPT),
			new HumanMessage(state.userInput),
		]);

		const { thinking: kwThinking, text: kwText } = extractThinking(keywordResponse.content);
		if (kwThinking) {
			callbacks.onStepDetail?.(`关键词提取 thinking:\n${kwThinking}`);
		}

		let searchKeywords: string[] = [];
		try {
			const clean = stripThinkTags(kwText.trim());
		...
		const filterResponse = await llm.invoke([
			new SystemMessage(FILTER_SYSTEM_PROMPT),
			new HumanMessage(`搜索意图：${state.userInput}\n\n候选笔记：\n${candidateInfo}`),
		]);

		const { thinking: filterThinking, text: filterText } = extractThinking(filterResponse.content);
		if (filterThinking) {
			callbacks.onStepDetail?.(`笔记筛选 thinking:\n${filterThinking}`);
		}

		let selectedPaths: string[] = [];
		try {
			const clean = stripThinkTags(filterText.trim());
			const jsonMatch = clean.match(/\[[^\]]*\]/);
			if (jsonMatch) {
